...
namespace Community
{
    ...
    public class ReplaceWithPrefab : EditorWindow
    {
        ...
        private Vector2 windowMinSize = new Vector2(300, 200);
        private Vector2 windowMaxSize = new Vector2(900, 1200);
        ...
        void OnGUI()
        {
            ...
            GUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                keepNames = GUILayout.Toggle(keepNames, "Keep names", EditorStyles.toolbarButton);
                keepRotation = GUILayout.Toggle(keepRotation, "Keep rotation", EditorStyles.toolbarButton);
                keepScale = GUILayout.Toggle(keepScale, "Keep scale", EditorStyles.toolbarButton);
                keepHierarchy = GUILayout.Toggle(keepHierarchy, "Keep Hierarchy", EditorStyles.toolbarButton);
                keepMaterials = GUILayout.Toggle(keepMaterials, "Keep Materials", EditorStyles.toolbarButton);
        ...
        void Replace(GameObject replacedObj)
        {
            ...
            newObject.transform.SetParent(replacedObj.transform.parent, true);

            newObject.transform.localPosition = replacedObj.transform.localPosition;

            if (keepMaterials)
            {
                Renderer[] replacedRenderers = replacedObj.GetComponentsInChildren<Renderer>();
                int count = replacedRenderers.Length;
                if (count > 0)
                {
                    Renderer[] newRenderers = newObject.GetComponentsInChildren<Renderer>();
                    if (newRenderers.Length == count)
                    {
                        for (int i = 0; i < count; i++)
                        {
                            newRenderers[i].sharedMaterials = replacedRenderers[i].sharedMaterials;
                        }
                        goto Succeeded;
                    }
                }
                Debug.LogWarning("Failed to keep materials..");
                Succeeded:;
            }

            if (keepRotation)
            {
                newObject.transform.localRotation = replacedObj.transform.localRotation;
            }

            if (keepScale)
            {
                newObject.transform.localScale = replacedObj.transform.localScale;
            }
            ...
            if (keepNames)
            {
                newObject.transform.name = replacedObj.transform.name;
            }

            if (keepHierarchy)
            {
                newObject.transform.SetSiblingIndex(replacedObj.transform.GetSiblingIndex());
            }

            Undo.RegisterCreatedObjectUndo(newObject, "Replaced Objects");
            Undo.DestroyObjectImmediate(replacedObj);
        }
